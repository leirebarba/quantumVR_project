...
SECTION_OUTPUT_DIR = get_output_dir("section1_plots")
...
def plot_test_scores_by_site(df):
    """Plot mean test scores before and after by site with error bars."""
    ...
    plt.tight_layout()
    plt.savefig(SECTION_OUTPUT_DIR / "test_scores_by_site.png", dpi=300)
    plt.close()


def plot_metric_grid(long_df):
    """Create a grid of bar plots comparing pre/post means for each metric by site."""
    ...
    for i, metric in enumerate(metrics):
        ...
        summary = sub.groupby(["site", "time"])["value"].agg(["mean", "std", "count"]).reset_index()
    ...
    plt.tight_layout()
    plt.savefig(SECTION_OUTPUT_DIR / "prepost_metric_grid.png", dpi=300, bbox_inches="tight")
    plt.close()


def plot_site_time_panels(df):
    """Create separate panels for each site and time point showing mean scores for all metrics."""
    ...
    for site in ["Madrid", "Segovia"]:
        for time in ["Before", "After"]:
            ...
            plt.savefig(SECTION_OUTPUT_DIR / filename, dpi=300)
            plt.close()
